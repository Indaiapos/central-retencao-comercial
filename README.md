...
- **O que é sincronizado:** produtos com `status = 'ativo'` e `deleted_at IS NULL`, agrupados por `categoria_nome`. Cada item vira `{ nome, valores: [...] }` a partir do campo `valor` do ERP — sem inventar um "valor praticado", porque esse conceito não existe no ERP (só existia na planilha antiga, hoje descontinuada). Se um produto está com `valor = 0` no ERP, aparece como R$ 0,00 aqui também — é reflexo fiel da fonte, não um bug.
- **Descontos padrão (calculados, não vêm do ERP):** duas categorias do ERP têm regra de desconto do comercial aplicada em cima do Valor Cheio, definida em `DECOR_CATEGORIAS`/`CARDAPIO_CATEGORIAS` no topo de `scripts/sync-precos-erp.js`. Decorações (`"Decorações - Item Adic."`, `"Decorações - Pacote"`) mostram `[Valor Cheio, Com 35% off, Com 60% off (máx.)]`; cardápio/comida (`"Cardápios"`, `"Cardápios - Item Adic."`, `"Buffet"`, `"Empratado"`, `"Brunch"`, `"Coffee Break"`, `"Coquetel"`, `"Finger Foods"`, `"Fondue / Especial"`, `"Menu Completo"`) mostram `[Valor Cheio, Com 10% off, Com 20% off]`. As demais categorias continuam com `[Valor Cheio, Mínimo]` vindo direto de `valor_minimo` do ERP. Para mudar o percentual ou quais categorias entram na regra, edite essas listas — o próximo sync já aplica.
- **Exceção "Sense":** dentro de "Decorações - Pacote", os produtos cujo nome começa com "Pacote Sense" ou "Quadro Sense" são preço fechado no ERP (`valor_minimo = valor`, sem faixa de negociação) — não entram na regra de desconto de decoração. `sync-precos-erp.js` os separa automaticamente numa categoria própria, `"Decorações - Pacote (Sense)"`, com `[Valor Cheio, Mínimo]` direto do ERP. Ver `SENSE_PATTERN` no script.
