...
- **Como rodar manualmente:** `node scripts/sync-precos-erp.js <dump.json>`, onde `<dump.json>` é o resultado bruto (formato `{ "linhas": [...] }`) de uma consulta tipo `SELECT nome, categoria_nome, valor, valor_minimo, unidade, desconto_maximo FROM produtos WHERE deleted_at IS NULL AND status = 'ativo' AND categoria_nome IS NOT NULL` via `indaia_sql`. O script recria inteiramente as categorias do grupo `"Produtos (ERP)"` — cria categoria nova se `categoria_nome` for novo, remove categoria que sumiu do ERP, atualiza itens/valores das que continuam — mas **preserva** `desc`, `sinonimos`, `icone` e `observacao` de categorias já existentes (casadas por `erpCategoriaNome`), então dá pra ajustar o texto pelo admin sem perder a edição no próximo sync.
- **Automático:** uma rotina agendada roda 1x por dia, consulta o ERP, roda o script e sobe o resultado pro GitHub sozinha — sem revisão humana a cada execução. Cadastrar um produto novo no ERP (ou mudar um preço) aparece no site em até 1 dia, sem precisar pedir para alguém mexer no código.
- Se um item específico ficar errado (nome ruim, categoria bagunçada no ERP), o ajuste certo é arrumar no ERP, não editar o valor à mão pelo admin — a próxima sincronização sobrescreve.
